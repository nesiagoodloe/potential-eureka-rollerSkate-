...
app.get('/', function (req, res) {
 ...
 app.post('/submitForm', (req, res) => {
  // Get personal info
  const { Fname, Lname, date, phonenum, Lnumber } = req.body;
  const personalInfo = { Fname, Lname, date, phonenum, Lnumber };

  // Get survey data
  const activities = req.body.activities || [];
  const landmark = req.body.landmark || '';
  const food = req.body.dest || '';
  const places = req.body.places || '';

  const survey = { activities, landmark, food, places };

  // Render the result page with both personal info and survey data
  res.render('result', { personalInfo, survey });
});
